...
public class ObjectDiskCacheItem<T> : IObjectCacheItem<T> where T : class
{
	...
	public T Value
	{
		get
		{
			lock (mSync)
			{
				if (mState is ItemState.Disposed or ItemState.SaveAndDisposePending)
	...
	public bool IsValueInMemory
	{
		get
		{
			lock (mSync)
			{
				...
				return mWeakReference is { IsAlive: true };
			}
		}
	}
	...
	public void TakeOwnership(IObjectCacheItem item)
	{
		...
		if (other.mState is ItemState.Disposed or ItemState.SaveAndDisposePending)
			throw new ObjectDisposedException(nameof(ObjectDiskCacheItem<T>));
	...
	private void SaveObject()
	{
		lock (mSync)
		{
			if (mState is not (ItemState.SavePending or ItemState.SaveAndDisposePending))
				return;
