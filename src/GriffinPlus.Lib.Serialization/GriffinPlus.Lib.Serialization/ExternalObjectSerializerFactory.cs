...
public static partial class ExternalObjectSerializerFactory
{
	...
	private static          List<SerializerInfo>                sRegisteredExternalObjectSerializers      = [];
	private static          TypeKeyedDictionary<SerializeeInfo> sExternalObjectSerializerInfoBySerializee = new();
	...
	public static bool TryRegisterExternalObjectSerializer(Type type)
	{
		lock (sSync)
		{
			...
			if (eosAttribute != null && eosBaseType != null && eosConstructor != null)
			{
				...
				{
					new(type, eosSerializeeType, eosAttribute.Version)
				};
			...
			if ((eosAttribute == null && eosBaseType == null) || !sLog.IsLogLevelActive(LogLevel.Error))
				return false;
	...
	private static List<Type> GetPossibleSerializeeTypes(Type type)
	{
		...
		Type[] implementedInterfaces = type.FindInterfaces((_, _) => true, null);
