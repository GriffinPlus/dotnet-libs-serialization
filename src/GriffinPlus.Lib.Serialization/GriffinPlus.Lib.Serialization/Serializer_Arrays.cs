...
partial class Serializer
{
	...
	private void WriteArray(bool[] array, IBufferWriter<byte> writer)
	{
		...
		if (SerializationOptimization == SerializationOptimization.Speed)
		{
			...
			bufferIndex += Leb128EncodingHelper.Write(buffer[bufferIndex..], array.Length);
...
#error Unhandled .NET framework.
	...
	private void WriteArray(char[] array, IBufferWriter<byte> writer)
	{
		...
		else
		{
			...
			bufferIndex += Leb128EncodingHelper.Write(buffer[bufferIndex..], array.Length);
			encoding.CopyTo(buffer[bufferIndex..]);
...
#error Unhandled .NET framework.
	...
	private void WriteArray(IReadOnlyList<decimal> array, IBufferWriter<byte> writer)
	{
		...
		bufferIndex += Leb128EncodingHelper.Write(buffer[bufferIndex..], array.Count);
		writer.Advance(bufferIndex);
		...
		while (fromIndex < array.Count)
		{
			...
			for (int i = 0; i < elementsToCopy; i++)
			{
				...
				MemoryMarshal.Cast<int, byte>(bits.AsSpan()).CopyTo(buffer[(i * elementSize)..]);
			}
...
#error Unhandled .NET framework
	...
	private unsafe void WriteMultidimensionalArrayOfBoolean(Array array, IBufferWriter<byte> writer)
	{
		...
		try
		{
			if (SerializationOptimization == SerializationOptimization.Speed)
			{
				...
				bufferIndex += Leb128EncodingHelper.Write(buffer[bufferIndex..], array.Rank); // number of dimensions
				for (int i = 0; i < array.Rank; i++)
				{
					// ...dimension information...
					int lowerBound = array.GetLowerBound(i);
					int count = array.GetLength(i);
					bufferIndex += Leb128EncodingHelper.Write(buffer[bufferIndex..], lowerBound); // lower bound of the dimension
					bufferIndex += Leb128EncodingHelper.Write(buffer[bufferIndex..], count);      // number of elements in the dimension
				}
	...
	private void WriteMultidimensionalArrayOfDecimal(Array array, IBufferWriter<byte> writer)
	{
		...
		for (int i = 0; i < array.Rank; i++)
		{
			...
			int count = array.GetLength(i);
			bufferIndex += Leb128EncodingHelper.Write(buffer[bufferIndex..], indices[i]); // lower bound of the dimension
			bufferIndex += Leb128EncodingHelper.Write(buffer[bufferIndex..], count);      // number of elements in the dimension
		}
		...
		while (remaining > 0)
		{
			...
			for (int i = 0; i < elementsToCopy; i++)
			{
				...
				MemoryMarshal.Cast<int, byte>(bits.AsSpan()).CopyTo(buffer[bufferIndex..]);
#elif NET5_0_OR_GREATER
...
#error Unhandled .NET framework
	...
	private Array ReadMultidimensionalArrayOfDecimal(Stream stream)
	{
		...
		for (int i = 0; i < array.Length; i++)
		{
			...
			decimal value = new(TempBuffer_Int32);
			array.SetValue(value, indices);
			IncrementArrayIndices(indices, array);
		}
